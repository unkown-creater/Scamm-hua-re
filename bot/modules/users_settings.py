...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rcl.conf. Timeout: 60 sec'],
            ...
            'lcaption': ['Leech Caption is the Custom Caption on the Leech Files Uploaded by the bot.\n You can change front of your file caption by sending code mentioned below 👇\n\n caption code formate:<code> <x>{filename}</x> </code> \n now copy that and replace the "x" with \n for Bold: <b>\n for italic: <i>\n for mono: <code>\n for regular and underlined: <u>', 'Send Leech Caption. You can add HTML tags Timeout: 60 sec'],
